...
5. **Walk the slice loop, delegating each stage** (NEVER reinvent — invoke the named skill):
   ...
   | 7 merge-gate | active profile's merge-gate role. **`opus` profile → the adversarial PANEL:** `Workflow({scriptPath: "<hub>/workflows/merge-gate-panel.js", args: {project, repoPath, baseRef, headRef, sliceId, specGlobs, stack, highStakes, ledgerPath}})` — 4 refute-biased Opus lenses (cross-task seams · spec-rule citation · regression/data-safety · gate compliance) → 3-refuter verify → GO/NO-GO. **Pass `ledgerPath`** (the project's accepted/known-backlog ledger, default `docs/health/accepted.md` — the same one the stage-7½ health-sweep reads): the panel reads it and **DROPS / down-rates to a nit any finding already accepted and routed there**, so a known, deferred item (e.g. hims' field_def-seed deploy gap, routed to the auth/migrations slice) stops re-consuming a should-fix slot on every slice that touches that surface. A *new* aspect of a tracked item is still fair game; suppression is only for the already-logged, already-routed finding. **This stage-7 instruction is the standing Workflow opt-in** for the gate. **If the panel returns `INCONCLUSIVE`** (an API-overload storm meant too few lenses actually ran) it is NOT a pass — rerun the panel or fall back to a single `merge-gate-reviewer`; never read panel silence as GO. Degrade to a single `merge-gate-reviewer` dispatch only if a Workflow can't run. | — |
   ...
   | 11 reconcile | memory + docs + board + **verify post-merge main CI green** + **reconcile the stakeholder demo script** (see below) + write the slice retro | — |
...
### Demo-script reconciliation (stage 11 — keep the stakeholder walkthrough true to the build)
A project's stakeholder **demo script** (the seeded-fixture roster + the scripted click-through that a
non-engineer follows to see the feature) is a build artifact that **goes stale the instant a slice changes
what a click does** — a former dead-end now advances, a "what's not built" line names a thing the slice just
built, a roster row sits at a state the slice removed. **At stage-11 reconcile, ask one question: "would the
demo click differently after this slice?" If yes, update the script in the same PR.** The trigger is *behavior
the demo would now exercise differently*, not whether a contradiction is literally provable — reconcile
proactively, the way the docs audit reconciles the spec/ADRs. This is a reconcile chore, not a merge gate.
The **merge-gate (stage 7) is the backstop, not the owner**: if reconcile misses it, the cross-seam / gate-
compliance lens flags a now-false demo step as a should-fix (it has done so on consecutive slices — hims S4a:
the script still called a now-advanceable state a dead-end; S4b: same class again). A merge-gate catch here is
the convention working, but the cheaper place is reconcile (hims S4c updated the closing beat proactively and
the gate found nothing stale).
...
**No CI configured (a steady state, not a failure).** A project may run with **no CI at all** — the workflow
deleted and no required status check (the user's deliberate call after a chronic budget block: hims removed
`ci.yml` outright, and a free private repo can't even add branch protection). Distinguish this from the
infra-only case above: there is **no run to read** (`gh run list` is empty — not a `startup_failure`), so don't
hunt for a failed run or frame the merge as "merge-through a blocked check." The gate becomes: **full local +
live verification (all stack gates, with DB-integration tests where the slice has them) + the stage-7 merge-gate
GO IS the merge bar** — record the verification in the retro exactly as the merge-through path does. `gh pr merge`
needs **no `--admin`** (there is no check to override; once-needed `--admin` was for a blocked *required* check).
This is **not** a soft auto-advance dodge: the local + live verification and the merge-gate still run in full —
only the GitHub-run reference is absent. Still confirm there's genuinely no branch protection before relying on
it (`gh api repos/<owner>/<repo>/branches/main/protection` → 404/403). (hims S4a deleted CI on no-billing; S4b
made the first merge with no `--admin`, confirming no branch protection; S4c merged the same way as the now-
normal path.)
